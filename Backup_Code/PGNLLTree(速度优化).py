...
def optimize_parameter(Y, epsilon=1e-8):
    N_true = Y[:, 0]
    S_true = np.clip(Y[:, 1], epsilon, None)
    
    lambda_opt = float(np.mean(N_true))
    
    def optimize_alpha(n, sumS, sumLogS, epsilon=1e-8):
        if n <= 0:
            return np.nan

        meanS = max(sumS / n, epsilon)
        meanLogS = sumLogS / n
    ...
    sumS = float(np.sum(S_true))
    sumLogS = float(np.sum(np.log(S_true)))
    alpha_opt = optimize_alpha(n, sumS, sumLogS, epsilon=epsilon)
    meanS = max(sumS / n, epsilon)
    gamma_opt = float(meanS / alpha_opt)
    
    return np.array([lambda_opt, float(alpha_opt), float(gamma_opt)])
