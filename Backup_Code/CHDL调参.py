...
# === 3. Define CSSE-Huber Loss Function ===
...
# === 4. Numerical Optimization Function ===
def optimize_leaf(Y, sigma1=1.0, sigma2=1.0, delta=1.0, epsilon=1e-8):
    N_true = Y[:, 0]
    S_true = Y[:, 1]
...
class CSSEHuberDualLossTree:
    ...
    def save_json(self, filepath, feature_names=None, ndigits=6, indent=2, ensure_ascii=False):
        j = self.export_json(feature_names=feature_names, ndigits=ndigits,
                         indent=indent, ensure_ascii=ensure_ascii)
        filepath = Path(filepath)
        filepath.parent.mkdir(parents=True, exist_ok=True)
        filepath.write_text(j, encoding="utf-8")
        return str(filepath)
...
print("Best Parameters:",best_chdl_params)
