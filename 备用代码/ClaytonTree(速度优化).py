...
def optimize_parameter(Y, eps=1e-8, theta_bounds=(1e-3, 50.0)):
    Y = np.asarray(Y)
    ...
    def poisson_nll_lambda(lam):
        return (m + k) - T / lam - m / np.expm1(lam)

    def poisson_nll_lambda_prime(lam):
        em1 = np.expm1(lam)
        return (T / (lam*lam)) + m * (np.exp(lam) / (em1*em1))

    x0 = max(T / max(m + k, 1), 1e-3)
    result = root_scalar(poisson_nll_lambda, fprime=poisson_nll_lambda_prime, x0=x0, method="newton")
    lambda_hat = float(result.root)

    # Gamma Part
    def optimize_alpha(n, sumS, sumLogS, eps=1e-8):
        if n <= 0:
            return np.nan

        meanS = max(sumS / n, eps)
        meanLogS = sumLogS / n
        s = np.log(meanS) - meanLogS

        if not np.isfinite(s) or s <= 0:
            return 1e6

        alpha0 = (3.0 - s + np.sqrt((s - 3.0) ** 2 + 24.0 * s)) / (12.0 * s)
        alpha0 = float(np.clip(alpha0, 1e-6, 1e6))

        def g(alpha):
            return np.log(alpha) - sp.digamma(alpha) - s

        def gprime(alpha):
            return 1.0 / alpha - sp.polygamma(1, alpha)
        ...
        return float(np.clip(sol.root, 1e-6, 1e6))

    S_plus = np.clip(S[I_plus], eps, None)
    n = int(S_plus.shape[0])
    sumS = float(np.sum(S_plus))
    sumLogS = float(np.sum(np.log(S_plus)))
    alpha_hat = optimize_alpha(n, sumS, sumLogS)
    meanS = max(sumS / n, eps)
    gamma_hat = float(meanS / alpha_hat)

    # ---------- Step 2: Copula (theta) optimization ----------
    # pre-calculate
    N_plus = N[I_plus]
    S_plus = S[I_plus]
    u = gamma_dist.cdf(S_plus, a=alpha_hat, scale=gamma_hat)
    u = np.clip(u, eps, 1.0 - eps)
    v_n   = (poisson.cdf(N_plus, mu=lambda_hat) - np.exp(-lambda_hat)) / (1.0 - np.exp(-lambda_hat))
    v_nm1 = (poisson.cdf(N_plus - 1, mu=lambda_hat) - np.exp(-lambda_hat)) / (1.0 - np.exp(-lambda_hat))
    v_n = np.clip(v_n, eps, 1.0 - eps)
    v_nm1 = np.clip(v_nm1, eps, 1.0 - eps)
    logu     = np.log(u)
    logv_n   = np.log(v_n)
    logv_nm1 = np.log(v_nm1)
    u_inv = 1.0 / u
    ...
    def copula_nll_theta(theta):
        ...
        inv_theta = 1.0 / theta
        u_m    = np.exp(-theta * logu)
        vn_m   = np.exp(-theta * logv_n)
        vnm1_m = np.exp(-theta * logv_nm1)
        common_n   = np.clip(u_m + vn_m   - 1.0, eps, None)
        common_nm1 = np.clip(u_m + vnm1_m - 1.0, eps, None)
        u_pow = u_inv * u_m
        expo  = -(inv_theta + 1.0)
        D1_n   = u_pow * (common_n   ** expo)
        D1_nm1 = u_pow * (common_nm1 ** expo)
        Delta_i = np.clip(D1_n - D1_nm1, eps, None)
        return float(-np.sum(np.log(Delta_i)))
